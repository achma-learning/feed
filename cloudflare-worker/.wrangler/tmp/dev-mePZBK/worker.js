...
// .wrangler/tmp/bundle-fwdPQp/strip-cf-connecting-ip-header.js
...
// src/worker.js
var RSS_FEEDS = [
  { id: "health-maroc", cat: "health-maroc", name: "Sant\xE9 Maroc", url: "https://news.google.com/rss/topics/CAAqIQgKIhtDQkFTRGdvSUwyMHZNR3QwTlRFU0FtWnlLQUFQAQ?hl=fr&gl=MA&ceid=MA:fr", color: "#16a34a" },
  ...
  { id: "map-general", cat: "map", name: "MAP G\xE9n\xE9ral", url: "https://www.mapnews.ma/ar/actualites/general/rss", color: "#0891b2" },
  { id: "map-regional", cat: "map", name: "MAP R\xE9gional", url: "https://www.mapnews.ma/ar/actualites/regional/rss", color: "#0d9488" },
  ...
  { id: "health-france", cat: "health-world", name: "Sant\xE9 France", url: "https://news.google.com/rss/topics/CAAqIQgKIhtDQkFTRGdvSUwyMHZNR3QwTlRFU0FtWnlLQUFQAQ?hl=fr&gl=FR&ceid=FR:fr", color: "#1d4ed8" },
  { id: "mit", cat: "mit", name: "MIT News", url: "https://news.mit.edu/rss/feed", color: "#475569" },
  { id: "has-actualites", cat: "has", name: "HAS Actualit\xE9s", url: "https://www.has-sante.fr/jcms/fc_2874902/fr/actualites?rss=true", color: "#7c3aed" },
  ...
  { id: "vidal-medicaments", cat: "vidal", name: "Vidal M\xE9dicaments", url: "https://www.vidal.fr/actualites/medicaments-et-produits-de-sante.xml", color: "#0369a1" },
  ...
  { id: "vidal-sante-publique", cat: "vidal", name: "Vidal Sant\xE9 Publique", url: "https://www.vidal.fr/actualites/sante-publique.xml", color: "#0284c7" },
...
var SCRAPE_SOURCES = [
  {
    id: "sante-gov-activites",
    cat: "sante-gov",
    name: "Min. Sant\xE9 Activit\xE9s",
    url: "https://www.sante.gov.ma/Pages/activites.aspx",
    color: "#15803d"
  },
  {
    id: "sante-gov-actualites",
    cat: "sante-gov",
    name: "Min. Sant\xE9 Actualit\xE9s",
    url: "https://www.sante.gov.ma/Pages/toutes_actualites.aspx",
    color: "#15803d"
  },
  {
    id: "ammps-actualites",
    cat: "sante-gov",
    name: "AMMPS Actualit\xE9s",
    url: "https://ammps.sante.gov.ma/actualites",
    color: "#059669"
  },
  {
    id: "hcp-publications",
    cat: "hcp",
    name: "HCP Publications",
    url: "https://www.hcp.ma/downloads/",
    color: "#b45309"
  }
];
var FAR_BASE = "https://revue.far.ma";
var FAR_PAGES = [1, 2, 3];
var worker_default = {
  async fetch(request, env, ctx) {
    ...
    try {
      switch (url.pathname) {
        ...
        case "/health":
          return jsonResponse({ status: "ok", time: (/* @__PURE__ */ new Date()).toISOString() });
        default:
          return jsonResponse({
...
__name(handleFeeds, "handleFeeds");
async function handlePrayer(request, ctx, url) {
  ...
  try {
    ...
    const resp = await fetchWithTimeout(habousUrl, 1e4);
...
function parseRSSItem(item, feed) {
  ...
  if (!title)
    return null;
  let image = "";
  const enc = item.match(/<enclosure[^>]+type=["']image[^"']*["'][^>]+url=["']([^"']+)["']/i) || item.match(/<enclosure[^>]+url=["']([^"']+)["'][^>]+type=["']image/i);
  if (enc)
    image = enc[1];
  if (!image) {
    const m = item.match(/<media:thumbnail[^>]+url=["']([^"']+)["']/i);
    if (m)
      image = m[1];
  }
  if (!image) {
    const m = item.match(/<media:content[^>]+(?:type=["']image|medium=["']image)[^>]+url=["']([^"']+)["']/i);
    if (m)
      image = m[1];
  }
  if (!image) {
    const m = item.match(/<media:content[^>]+url=["']([^"']+)["'][^>]+(?:type=["']image|medium=["']image)/i);
    if (m)
      image = m[1];
  }
  if (!image) {
...
__name(parseRSSItem, "parseRSSItem");
function parseAtomEntry(entry, feed) {
  const title = extractCDATA(entry, "title");
  const link = extractAttr(entry, 'link[rel="alternate"]', "href") || extractAttr(entry, "link", "href");
  ...
  return {
    title: stripHTML(title),
    link: link || "",
    pubDate: updated || (/* @__PURE__ */ new Date()).toISOString(),
    image,
    source: feed.name,
    sourceId: feed.id,
    cat: feed.cat,
...
__name(parseAtomEntry, "parseAtomEntry");
...
__name(scrapeAllHTML, "scrapeAllHTML");
async function scrapeSingleHTML(src) {
  const resp = await fetchWithTimeout(src.url, 15e3);
  if (!resp.ok)
    throw new Error(`HTTP ${resp.status}`);
  const html = await resp.text();
  switch (src.id) {
...
__name(scrapeSingleHTML, "scrapeSingleHTML");
...
function scrapeVidal(feed, html) {
  ...
  while ((m = pattern.exec(html)) !== null) {
    ...
    if (title.length > 15 && !seen.has(link)) {
      ...
      if (imgMatch)
        image = imgMatch[1].startsWith("http") ? imgMatch[1] : "https://www.vidal.fr" + imgMatch[1];
      articles.push({
        title,
        link,
        image,
...
__name(scrapeVidal, "scrapeVidal");
function scrapeSanteGov(html, src) {
  ...
  while ((m = pattern.exec(html)) !== null) {
    ...
    if (!link.startsWith("http"))
      link = "https://www.sante.gov.ma" + (link.startsWith("/") ? "" : "/") + link;
...
__name(scrapeSanteGov, "scrapeSanteGov");
function scrapeAMMPS(html, src) {
  ...
  while ((m = pattern.exec(html)) !== null) {
    ...
    let link = m[1];
    if (!link.startsWith("http"))
      link = "https://ammps.sante.gov.ma" + (link.startsWith("/") ? "" : "/") + link;
    if (title.length > 10 && !seen.has(link)) {
      seen.add(link);
      articles.push({
        title,
...
__name(scrapeAMMPS, "scrapeAMMPS");
function scrapeHCP(html, src) {
  ...
  while ((m = pattern.exec(html)) !== null) {
    ...
    let link = m[1];
    if (!link.startsWith("http"))
      link = "https://www.hcp.ma" + (link.startsWith("/") ? "" : "/") + link;
    if (title.length > 10 && !seen.has(link)) {
      seen.add(link);
      articles.push({
        title,
...
async function scrapeFarRevues() {
  ...
  for (const r of results) {
    if (r.status === "fulfilled")
      allRevues.push(...r.value);
  }
  return allRevues;
}
__name(scrapeFarRevues, "scrapeFarRevues");
async function scrapeFarPage(page) {
  const resp = await fetchWithTimeout(`${FAR_BASE}/revues?page=${page}`, 15e3);
  if (!resp.ok)
    throw new Error(`HTTP ${resp.status}`);
  const html = await resp.text();
  return parseFarHTML(html);
}
__name(scrapeFarPage, "scrapeFarPage");
function parseFarHTML(html) {
  ...
  while ((cardMatch = cardPattern.exec(html)) !== null) {
    ...
    if (!title) {
      ...
      if (aTextMatch)
        title = stripHTML(aTextMatch[1]).trim();
    }
    if (title && link) {
...
__name(parseFarHTML, "parseFarHTML");
...
__name(parseHabousPrayer, "parseHabousPrayer");
...
__name(extractCDATA, "extractCDATA");
...
function stripHTML(str) {
  if (!str)
    return "";
  return str.replace(/<!\[CDATA\[([\s\S]*?)\]\]>/g, "$1").replace(/<[^>]+>/g, "").replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&#39;/g, "'").replace(/&nbsp;/g, " ").replace(/\s+/g, " ").trim();
}
...
// .wrangler/tmp/bundle-fwdPQp/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-fwdPQp/middleware-loader.entry.ts
